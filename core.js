...
Core.prototype.loadWarriors = function (warriors) {
  // optimistically pick n spots and hope they are each length + mindistance apart
  var loadPoints = [];
  while (loadPoints.length < warriors.length) {
    var rnd = Math.floor(Math.random() * (this.coresize + 1));
    var viable = true;
    for (var i in loadPoints) {
      if (Math.abs(loadPoints[j] - rnd) < this.mindistance) {
        viable = false;
      }
    }
    if (viable) {
  ...
  for (var i in warriors) {
    ...
    for (var j in instructions) {
      this.core[loadPoint + j] = instructions[j];
    }
...
Core.prototype.setDefaultCommandModifiers = function (cmd) {
...
Core.prototype.runOnce = function () {
  var warrior = this.processes[this.nextWarrior];
  this.nextWarrior = modinc(this.nextWarror, 1, this.processes.length);
  
  var process = warrior.next();
  var position = process.position;
  var inst = this.core[position];
  var cmd = inst[CMD];
  cmd = this.setDefaultCommandModifiers(cmd);
...
// to generate this from the list above
// cat core.js | sed -E "s/var (.+) = .*;/exports.\1 = \1;/"
